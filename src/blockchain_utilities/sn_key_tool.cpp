...
generate [--overwrite] FILENAME
    ...
    For an active service node this file is named `key_ed25519` in the sispopd
    data directory.
...
legacy [--overwrite] FILENAME
    ...
    Note that legacy keypairs are not needed as of Sispop 8.x; you can use just a
...
int generate(bool ed25519, std::list<std::string_view> args) {
    ...
    if (pubkey_pos != std::string::npos)
        filename.replace(pubkey_pos, 6, sispopmq::to_hex(pubkey.begin(), pubkey.end()));
...
int show(std::list<std::string_view> args) {
    ...
    if (legacy) {
        ...
        std::cout << filename.u8string() << " (legacy SN keypair)" << "\n==========" <<
            "\nPrivate key: " << sispopmq::to_hex(seckey.begin(), seckey.begin() + 32) <<
            "\nPublic key:  " << sispopmq::to_hex(pubkey.begin(), pubkey.end()) << "\n\n";
        return 0;
    }
    ...
    if (size >= 64 && ustring_view{pubkey.data(), pubkey.size()} != ustring_view{seckey.data() + 32, 32})
        return error(13, "Error: derived pubkey (" + sispopmq::to_hex(pubkey.begin(), pubkey.end()) + ")"
                " != embedded pubkey (" + sispopmq::to_hex(seckey.begin() + 32, seckey.end()) + ")");
    if (0 != crypto_sign_ed25519_pk_to_curve25519(x_pubkey.data(), pubkey.data()))
...
int restore(bool ed25519, std::list<std::string_view> args) {
    ...
    if (!(skey_hex.size() == 64 || skey_hex.size() == 128) || !sispopmq::is_hex(skey_hex))
    ...
    sispopmq::from_hex(skey_hex.begin(), skey_hex.begin() + 64, seed.begin());
    if (skey_hex.size() == 128)
        sispopmq::from_hex(skey_hex.begin() + 64, skey_hex.end(), pubkey_expected.emplace().begin());
    ...
    if (pubkey_expected) {
        if (*pubkey_expected != pubkey)
            return error(2, "Derived pubkey (" + sispopmq::to_hex(pubkey.begin(), pubkey.end()) + ") doesn't match "
                    "provided pubkey (" + sispopmq::to_hex(pubkey_expected->begin(), pubkey_expected->end()) + ")");
    } else {
