...
namespace cryptonote::rpc {
  ...
  // Invokes the actual RPC request; this is called (via sispopmq) from some random LMQ worker thread,
  ...
  static std::unordered_set<sispopmq::SispopMQ*> timer_started;
  ...
  void http_server::start()
  {
    ...
    m_server_header = "sispopd/"s + (m_restricted ? std::to_string(SISPOP_VERSION[0]) : std::string{SISPOP_VERSION_FULL})
