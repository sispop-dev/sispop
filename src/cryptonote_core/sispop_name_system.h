...
inline std::ostream &operator<<(std::ostream &os, mapping_value const &v) { return os << sispopmq::to_hex(v.to_view()); }
...
struct name_system_db
{
  ...
  bool                        save_mapping   (crypto::hash const &tx_hash, cryptonote::tx_extra_sispop_name_system const &src, uint64_t height, std::optional<uint64_t> expiration_height, int64_t owner_id, std::optional<int64_t> backup_owner_id);
  ...
  bool validate_ons_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_sispop_name_system &entry, std::string *reason);
