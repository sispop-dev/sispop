...
std::string lns::mapping_value::to_readable_value(cryptonote::network_type nettype, lns::mapping_type type) const
{
  ...
  {
    result = sispopmq::to_base32z(to_view());
  }
...
std::string lns_extra_string(cryptonote::network_type nettype, cryptonote::tx_extra_sispop_name_system const &data)
{
  ...
  if (data.is_buying())
  {
    ...
    stream << "signature=" << sispopmq::to_hex(tools::view_guts(data.signature.data));
...
bool validate_mapping_value(cryptonote::network_type nettype, mapping_type type, std::string const &value, mapping_value *blob, std::string *reason)
{
  ...
  else if (is_sispopnet_type(type))
  {
    ...
    sispopmq::to_hex(value.begin(), value.end(), pkey.data);
...
{
  return sispopmq::to_base64(tools::view_guts(hash));
}
