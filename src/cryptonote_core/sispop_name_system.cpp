...
#define SISPOP_DEFAULT_LOG_CATEGORY "ons"
...
std::string ons::mapping_value::to_readable_value(cryptonote::network_type nettype, ons::mapping_type type) const
{
  ...
  if (is_lokinet_type(type))
  {
    ...
    if(addr)
    {
      ...
      result = sispopmq::to_hex(to_view());
    }
  } else {
    result = sispopmq::to_hex(to_view());
  }
...
std::string ons_extra_string(cryptonote::network_type nettype, cryptonote::tx_extra_sispop_name_system const &data)
{
...
bool parse_owner_to_generic_owner(cryptonote::network_type nettype, std::string_view owner, generic_owner &result, std::string *reason)
{
  ...
  else if (owner.size() == 2*sizeof(ed_owner.data) && sispopmq::is_hex(owner))
  {
    sispopmq::from_hex(owner.begin(), owner.end(), ed_owner.data);
...
std::optional<cryptonote::address_parse_info> encrypted_wallet_value_to_info(std::string name, std::string encrypted_value, std::string nonce)
{
  ...
  mapping_value record(sispopmq::from_hex(encrypted_value), sispopmq::from_hex(nonce));
...
static bool check_lengths(mapping_type type, std::string_view value, size_t max, bool binary_val, std::string *reason)
{
  ...
  if (!result)
  {
    if (reason)
    {
      ...
      if (binary_val) err_stream << sispopmq::to_hex(value);
...
bool mapping_value::validate(cryptonote::network_type nettype, mapping_type type, std::string_view value, mapping_value *blob, std::string *reason)
{
  ...
  else if (is_lokinet_type(type))
  {
    ...
    if (blob)
    {
      ...
      sispopmq::from_base32z(value.begin(), value.begin() + 52, blob->buffer.begin());
    }
  }
  else
  {
...
std::string name_hash_bytes_to_base64(std::string_view bytes)
{
  ...
  return sispopmq::to_base64(bytes);
}
...
std::optional<std::string> name_hash_input_to_base64(std::string_view input)
{
  ...
  if (input.size() == 2*NAME_HASH_SIZE && sispopmq::is_hex(input))
    return name_hash_bytes_to_base64(sispopmq::from_hex(input));
  if (input.size() >= NAME_HASH_SIZE_B64_MIN && input.size() <= NAME_HASH_SIZE_B64_MAX && sispopmq::is_base64(input)) {
    std::string tmp = sispopmq::from_base64(input);
...
static bool validate_against_previous_mapping(ons::name_system_db &ons_db, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_sispop_name_system const &ons_extra, std::string *reason)
{
  std::stringstream err_stream;
...
bool name_system_db::validate_ons_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_sispop_name_system &ons_extra, std::string *reason)
{
  // -----------------------------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------------------------
  {
    ...
    if (check_condition(!cryptonote::get_field_from_tx_extra(tx.extra, ons_extra), reason, tx, ", didn't have sispop name service in the tx_extra"))
      return false;
  }
  ...
  // -----------------------------------------------------------------------------------------------
  {
    ...
    if (burn != burn_required)
    {
      ...
      if (check_condition(true, reason, tx, ", ", ons_extra_string(nettype, ons_extra), " burned ", over_or_under, "sispop=", burn, ", require=", burn_required))
        return false;
    }
  }
...
std::optional<int64_t> add_or_get_owner_id(ons::name_system_db &ons_db, crypto::hash const &tx_hash, cryptonote::tx_extra_sispop_name_system const &entry, ons::generic_owner const &key)
{
  int64_t result = 0;
...
std::pair<std::string, std::vector<update_variant>> update_record_query(name_system_db& ons_db, uint64_t height, const cryptonote::tx_extra_sispop_name_system& entry, const crypto::hash& tx_hash)
{
...
bool add_ons_entry(ons::name_system_db &ons_db, uint64_t height, cryptonote::tx_extra_sispop_name_system const &entry, crypto::hash const &tx_hash)
{
  // -----------------------------------------------------------------------------------------------
...
struct ons_update_history
{
  ...
  void     update(uint64_t height, cryptonote::tx_extra_sispop_name_system const &ons_extra);
  uint64_t newest_update_height() const;
};

void ons_update_history::update(uint64_t height, cryptonote::tx_extra_sispop_name_system const &ons_extra)
{
  if (ons_extra.field_is_set(ons::extra_field::encrypted_value))
...
bool name_system_db::save_mapping(crypto::hash const &tx_hash, cryptonote::tx_extra_sispop_name_system const &src, uint64_t height, std::optional<uint64_t> expiration, int64_t owner_id, std::optional<int64_t> backup_owner_id)
{
  if (!src.is_buying())
    return false;
...
{
  assert(name_base64_hash.size() == 44 && name_base64_hash.back() == '=' && sispopmq::is_base64(name_base64_hash));
  mapping_record result = {};
...
{
  assert(name_hash_b64.size() == 44 && name_hash_b64.back() == '=' && sispopmq::is_base64(name_hash_b64));
