...
`--tx-proxy` which tells `sispopd` the ip/port of a socks proxy provided by a
...
`sispopd` currently selects two outgoing connections every 5 minutes for
...
`sispopd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `sispopd` should likely disconnect
