...
local debian_pipeline(name, image,
        ...
        werror=false, // FIXME
        build_tests=true,
        test_sispopd=true, # Simple sispopd offline startup test
        ...
        submodules,
        {
            ...
            commands: [
                ...
                'eatmydata ' + apt_get_quiet + ' install -y --no-install-recommends cmake git ca-certificates ninja-build ccache '
                    + deps + (if test_sispopd then ' gdb' else ''),
                'mkdir build',
                'cd build',
                'cmake .. -G Ninja -DCMAKE_CXX_FLAGS=-fdiagnostics-color=always -DCMAKE_BUILD_TYPE='+build_type+' ' +
                    '-DLOCAL_MIRROR=https://builds.sispopnet.dev/deps -DUSE_LTO=' + (if lto then 'ON ' else 'OFF ') +
                ...
                if test_sispopd then [
                    '(sleep 3; echo "status\ndiff\nexit") | TERM=xterm ../utils/build_scripts/drone-gdb.sh ./bin/sispopd --offline --data-dir=startuptest'
                ] else []
            ) + (
                if run_tests then [
                    'mkdir -v -p $$HOME/.sispop',
...
local android_build_steps(android_abi, android_platform=21, jobs=6, cmake_extra='') = [
    ...
    'cmake .. -DCMAKE_CXX_FLAGS=-fdiagnostics-color=always -DCMAKE_C_FLAGS=-fdiagnostics-color=always ' +
        ...
        '-DLOCAL_MIRROR=https://builds.sispopnet.dev/deps ' +
...
local gui_wallet_step(image, wine=false) = {
    ...
    commands: [
        ...
        'cp -v build/bin/sispopd' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
        'cp -v build/bin/sispop-wallet-rpc' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
    ...
    commands: [
        'git clone https://github.com/loki-project/loki-electron-gui-wallet.git',
        'cp -v build/bin/{sispopd,sispop-wallet-rpc} loki-electron-gui-wallet/bin',
        'cd loki-electron-gui-wallet',
        'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
        'npm install',
        'npm run build',
...
[
    ...
    // Static build (on bionic) which gets uploaded to builds.sispopnet.dev:
    ...
    // Static mingw build (on focal) which gets uploaded to builds.sispopnet.dev:
    debian_pipeline("Static (win64)", "ubuntu:focal", deps='g++ g++-mingw-w64-x86-64 '+static_build_deps,
                    ...
                    build_tests=false, lto=false, test_sispopd=false, extra_cmds=[
